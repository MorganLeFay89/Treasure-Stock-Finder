...
| 機能 | 状態 | 説明 |
|------|------|------|
| 📊 条件検索 | ✅ 実装済 | 売上高成長率・PER・PBR・PSR・PEG・配当利回り・自己資本比率等を範囲指定してローカル高速スクリーニング |
| 🔄 バックグラウンド統合同期 | ✅ 実装済 | 5リクエスト/分以下の制限を守りJ-Quants・EDINETデータを銘柄順に自動取得・保存＆全件更新ローリング |
...
本アプリは、利用者が個人として契約したJ-Quants APIを、利用者本人の私的利用のために使用するローカルアプリケーションです。
利用者は、ご自身でJ-Quants APIの利用登録およびプラン契約を行い、ご自身に発行されたAPIキーを使用してください。
APIキー、取得データおよび分析結果は利用者の端末内で処理され、当社のサーバーには送信されません。
取得したデータおよび分析結果の公開、配布、第三者への提供、業務利用その他J-Quants APIサービス利用規約に反する利用は禁止されています。
本ソフトは株式会社JPX総研または日本取引所グループが提供、推奨もしくは保証する製品ではありません。利用者はJ-Quants APIの最新の利用規約を確認し、これを遵守するものとします。
